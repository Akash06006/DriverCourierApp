                ...
                when (it) {
                    "btn_submit" -> {
                        when {
                            ...
                            password.length < 8 -> showPasswordError(MyApplication.instance.getString(R.string.password_len_msg))
                            confirm.isEmpty() -> showConfirmPasswordError(
                                MyApplication.instance.getString(R.string.empty) + " " + MyApplication.instance.getString(
                                    R.string.confirm_password
                                )
                            )
                            confirm.length < 8 -> showConfirmPasswordError(MyApplication.instance.getString(R.string.password_len_msg))
                            password != confirm -> showConfirmPasswordError(MyApplication.instance.getString(R.string.mismatch_paaword))
